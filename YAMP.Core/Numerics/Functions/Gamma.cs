...
namespace YAMP.Numerics
{
	...
	public static class Gamma
    {
        ...
        {
            return Complex.Exp(LogGamma(a) + LogGamma(b) - LogGamma(a + b));
        }
		...
		{
			if (z.Im < 0.0)
				return LogGamma_Stirling(z.Conj()).Conj();

			var f = (z - 0.5) * Complex.Ln(z) - z + Math.Log(2.0 * Math.PI) / 2.0;
			var reduce = f.Im / (2.0 * Math.PI);
			reduce = f.Im - (int)(reduce) * 2.0 * Math.PI;
            f = new Complex(f.Re, reduce);
        ...
        static Complex LanczosLogGamma(Complex z)
		{
            Complex sum = Helpers.LanczosD[0];
			...
			var zshift = z + 0.5;
            var t = zshift * Complex.Ln(zshift + Helpers.LanczosR) - z;

			return t + Complex.Ln(sum);
		}
