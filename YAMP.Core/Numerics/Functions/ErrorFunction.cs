	...
	public static class ErrorFunction
    {
        ...
        public static Complex Erf(Complex z)
        {
            if (z.Abs() < 4.0)
                ...
                return Complex.Exp(-z * z) * Faddeeva(-Complex.I * z) - 1.0;

            return 1.0 - Complex.Exp(-z * z) * Faddeeva(Complex.I * z);
        }
        ...
        {
            if (z.Im < 0.0) 
                return 2.0 * Complex.Exp(-z * z) - Faddeeva(-z);

            if (z.Re < 0.0)
                return Faddeeva(-z.Conj()).Conj();

            var r = z.Abs();

            if (r < 2.0)
                return Complex.Exp(-z * z) * (1.0 - Erf_Series(-Complex.I * z));
            else if ((z.Im < 0.1) && (z.Re < 30.0))
                return Taylor(z.Re, Math.Exp(-z.Re * z.Re) + 2.0 * Dawson.DawsonIntegral(z.Re) / Helpers.SqrtPI * Complex.I, new Complex(0.0, z.Im));
            else if (r > 7.0)
        ...
        static Complex Weideman(Complex z)
        {
            var ZN = WeidemanL + Complex.I * z;
            var ZD = WeidemanL - Complex.I * z;
            var ZQ = ZN / ZD;
            Complex f = WeidemanCoefficients[40];
        ...
        static Complex ContinuedFraction(Complex z)
        {
            var a = 1.0;			// a_1
            var b = z;	            // b_1
            ...
            for (int k = 1; k < 250; k++)
            {
                ...
                if (f == f_old)
                    return Complex.I / Helpers.SqrtPI * f;
            }

            return Complex.I / Helpers.SqrtPI * f;
        }

        static Complex Taylor(Complex z0, Complex w0, Complex dz)
        {
            // first order Taylor expansion
            var wp_old = w0;
            var wp = 2.0 * (Complex.I / Helpers.SqrtPI - z0 * w0);
            var zz = dz;
            var w = w0 + wp * dz;
